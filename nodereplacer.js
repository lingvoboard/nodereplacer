...
node nodereplacer.js -retag (|-(gls|e)) input.txt output.txt
...
class offset_eol {
	constructor(inputfile, in_encoding, outputfile, out_encoding, fileSize, writer_buffering_size) {
		...
		try {
			...
			if (inbuffer !== undefined) {
				...
				if (len) {
					...
					if (n) {
						var size = n[0] - (offset + Buffer.byteLength(eol, this.in_encoding));
					} else {
						var size = this.fileSize - (offset + Buffer.byteLength(eol, this.in_encoding));
					}
	...
	write(str) {
		fs.writeFileSync(this.outputfile, str, { encoding: this.out_encoding, flag: 'a' });
	}
}
...
function buildfunctionbody(InputFileName, cb) {
	...
	const Output = fs.openSync(__dirname + '/files/rep_modules/main/index.js', 'w');
	...
	reader
		.on('line', line => {
			...
			if (/^\/\//.test(line) === true) {
				...
				code += 's = s.replace(';
				code += '/' + escapeSlash(m[1]) + '/umg' + m[2] + ', ' + '"' + escapeDQ(m[3]) + '"';
				code += ');\n';
			} else {
				code += line + '\n';
			}
...
function byline() {
	...
	if (o.eol_mode > 0) {
		delete o.idata;
		o.idata = new offset_eol(o.inputfile, o.in_encoding, o.outputfile, o.out_encoding, fileSize, 100);
	...
	reader
		...
		.on('close', () => {
			if (o.eol_mode > 0) {
				o.idata.file_end();
				o.idata.save_reader_disk_access_counter(eol_reader.get_disk_access_counter());
			}
...
function ProcessDSLArticle(art0, art1, o, art_num, art_start) {
	...
	//Заголовков нет
	if (info[0] === 0) {
		fs.writeFileSync(o.error_log_path, `Error: Empty entry headword. Line: ${art_start}\n`, {
			encoding: 'utf8',
			flag: 'a'
		});
		errorCount++;
	...
	//Тела нет
	if (info[1] === 0) {
		fs.writeFileSync(o.error_log_path, `Error: Empty card. Line: ${art_start}\n`, {
			encoding: 'utf8',
			flag: 'a'
		});
		errorCount++;
				...
				if (line === '') {
					if (
						...
						if (o.loop === 1 && arr.length !== 0) {
							fs.writeFileSync(o.error_log_path, 'Format error on line ' + art_start + '\n', {
								encoding: 'utf8',
								flag: 'a'
							});
							errorCount++;
...
function symbrep(s) {
	...
	for (let i = 0; i < temparr.length; i++) {
		const rm = new RegExp('^(\\[' + name + '\\])([\\s\\S]*)(\\[\\/' + name + '\\])$', 'im');
	...
	if (res !== true) {
		if (o.inputfile === null || o.outputfile === null) {
			...
			if (o.outputfile !== null) {
				fs.writeFileSync(o.outputfile, '', { encoding: o.out_encoding, flag: 'w' });

				for (let i = 0; i < o.res.length; i++) {
					if (o.res[i] !== null) {
						fs.writeFileSync(o.outputfile, o.res[i] + o.eol, { encoding: o.out_encoding, flag: 'a' });
					}
				}
			}
...
switch (process.argv.length) {
	...
	case 6:
		...
		if (
			...
			(pluginPath = __dirname + '/files/plugins/' + process.argv[2].substring(1) + '.js') &&
	...
	case 3:
		if (/^.+\.(?:gls|ifo)$/i.test(process.argv[2]) && fileExists(process.argv[2])) {
			//stardict.js
...
if (
	...
	/^-(rd|rt)$/.test(process.argv[2]) === true &&
	fileExists(process.argv[3]) &&
	fileExists(process.argv[4])
) {
	...
	/^-(rt|rd|rs|rg|re)$/i.test(process.argv[2]) === false &&
	fileExists(__dirname + '/files/plugins/' + process.argv[2].substring(1) + '.js') &&
	fileExists(process.argv[4])
) {
	//node nodereplacer.js -ключ1 -ключ2 input.txt output.txt
	buildfunctionbody(__dirname + '/files/plugins/' + process.argv[2].substring(1) + '.js', function() {
		prepare(process.argv[4], process.argv[5]);
	});
} else if (
...
} else if (process.argv.length === 3 && fileExists(process.argv[2]) && /^.+\.(?:gls|ifo)$/i.test(process.argv[2])) {
	//stardict.js
	buildfunctionbody(__dirname + '/files/' + 'stardict.js', function() {
		prepare(process.argv[2], null);
	});
} else if (process.argv.length === 3 && fileExists(process.argv[2]) && /^.+\.js$/i.test(process.argv[2])) {
	//node nodereplacer.js test.js
	buildfunctionbody(process.argv[2], function() {
		prepare(null, null);
	});
