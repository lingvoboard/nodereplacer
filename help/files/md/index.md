...
![](https://s10.postimg.org/ffb9ed3e1/warning.png)
...
Замены в файле имеющем [словарную структуру](#1_1078) используя список замен (**_list.txt_**).

###### 2) node nodereplacer.js (-rt|-re) list.txt input.txt output.txt
Замены в файле **не** имеющем словарную структуру используя список замен (**_list.txt_**).
С ключом -**_rt_** входной файл читается и обрабатываться построчно.
...
Замены в файле в формате [GLS](#2_1081) используя список замен (list.txt).

###### 4) node nodereplacer.js -rs list.txt -[A-Za-z]{1,10} input.txt output.txt
Замены между указанными DSL тэгами.
...
[b] - _node nodereplacer.js -rs list.txt -b input.txt output.txt_
...
Конвертирование словарей формате GLS в формат StarDict и обратно.
Поддерживается только [sametypesequence=h](./files/StarDictFileFormat.txt)


Варианты командной строки:
node nodereplacer.js словарь.gls
...
[dictzip.exe](https://github.com/Tvangeste/dictzip-win32/releases) для Windows можно найти в архиве со скриптом.
Перенесите его папку C:\Windows\System32\

CREDITS: [vsemozhetbyt](https://gist.github.com/vsemozhetbyt)
...
Объединение в словаре разных статей с одинаковыми заголовками с предварительным удалением дубликатов статей.
Входной файл должен иметь словарную структуру.
...
Объединение статей в словаре имеющих одинаковое тело со складыванием заголовков в стопы.
Входной файл должен иметь словарную структуру.
...
cane
        собака
```
Обратный слеш перед перед квадратными и круглыми скобками,
пробелы и табуляторы на конце заголовка скриптом игнорируются.
...
pig
        свинья 1
```
-**bi** - _сортировать статьи по заголовкам с учетом содержимого тела карточки.
И при сортировке игнорировать в заголовках:_
а) _фигурные скобки и все, что находится между ними_
б) _обратный слэш перед символами @\#^~[]{}()_
...
pig
        свинья 2
```
-**bie** - _это расширенный вариант -**bi**.
При сортировке игнорируются также_:
а) _символы: «»"„“”‘’…_
б) _многоточие из обычных точек (это помимо Horizontal ellipsis (U+2026))_
в) _диакритические знаки в символах ё и Ё._
г) _круглые скобки и все, что находится между ними_

-**oi** - _сортировать статьи по заголовкам, но с сохранением оригинального порядка в блоке статей имеющих одинаковый заголовок.
И при сортировке игнорировать в заголовках_:
а) _фигурные скобки и все, что находится между ними_
б) _обратный слэш перед символами @\#^~[]{}()_
...
pig
        свинья 1
```
-**oie** - _это расширенный вариант -**oi**.
При сортировке игнорируются также:_
а) _символы: «»"„“”‘’…_
б) _многоточие из обычных точек (это помимо Horizontal ellipsis (U+2026))_
в) _диакритические знаки в символах ё и Ё._
г) _круглые скобки и все, что находится между ними_

-**bd** - _то же самое, что и -**b**, но с игнорированием диакритических знаков в заголовках_
Входной файл должен иметь словарную структуру.
...
z
        aaa
```
-**od** - _то же самое, что и -o, но с игнорированием диакритических знаков в заголовках_
Входной файл должен иметь словарную структуру.
...
z
        aaa
```
-**bdc** - _то же самое, что и -bd, но с игнорированием регистра_
Входной файл должен иметь словарную структуру.
...
Mḁlta 3
        aaa
```
-**odc** - _то же самое, что и -**od**, но с игнорированием регистра_
Входной файл должен иметь словарную структуру.

###### 13) node nodereplacer.js (-usort|-usort -i) input.txt output.txt
Пользовательская сортировка.
Входной файл должен иметь словарную структуру (в формате [DSL](#1_1078)).
Файл с правилами сортировки **_sortRules.txt_** должен лежать в одной папке со скриптом. Его наличие опционально.

Следующие символы скрипт из **_sortRules.txt_** удаляет:
\x20\f\n\r\t\v\u00a0\u1680\u180e\u2000-\u200a\u2028\u2029\u202f\u205f\u3000\ufeff
...
![](https://s10.postimg.org/qjvmm1wcp/multibyte.png)

###### 14) node nodereplacer.js -stags input.txt output.txt
Сортировка DSL тегов.
Входной файл должен иметь словарную структуру.

Скрипт сначала сортирует открывающие теги в зависимости от положения закрывающих тегов.
...
**а**) Внедрение картинок в HTML файл (-**_base64_** -**_i_**).
Наличие словарной структуры у входного файла не обязательно.
Картинки должны находиться в папке "res", а папка "res" в одной папке с входным файлом.
...
-**dsl1** и -**dsl2** - _для обработки словарей в формате_ [DSL](#1_1078)
...
-**gls** - _для обработки словарей в формате_ [GLS](#2_1081)
...
Входной файл должен быть в формате [GLS](#2_1081).
