...
/******************************************************************************/
function gls2sd() {
  o.inDic.rli = o.rl.createInterface({ input: fs.createReadStream(o.fpath, o.inCoding) });
    ...
    if (!line) {
      ...
      if (o.inDic.article.length === 0) {
        if (o.inDic.empties.length) {
          ...
          if (!headwords.length) {
            abortOnParsingError(
              `Error: no headwords in the article at line ${o.inDic.lineNumber}.`
            );
          }

          if (headwords.some(hw => o.getByteLength(hw, o.inCoding) >= o.HEADWORD_EDGE)) {
...
function writeSDDic() {
  ...
  o.inDic.dic.forEach((artcl, i) => { artcl.push(i); }); // for stable sorting
  ...
  o.inDic.dic.forEach((artcl, i) => {
    ...
    fs.writeSync(o.outDic.dictFile,
      `${def}`,
    null, o.outCoding);
...
function writeSDIfo() {
  ...
  fs.writeFileSync(o.fpath.replace(o.changeExtRE, '.ifo'), xs`
    StarDict's dict ifo file
    version=2.4.2
    bookname=${o.inDic.directives['Glossary title']||o.path.basename(o.fpath, o.fext)}
    ...
    ${Object.keys(o.inDic.directives)
      ...
      .map(k => `${k.toLowerCase().replace(/ /g, '_')}=${o.inDic.directives[k]}`)
      .join('\n')
    }
...
function sd2gls() {
  ...
  o.inDic.needFiles = ['.ifo', '.idx', '.idx.gz', '.dict', '.dict.dz', '.syn']
                    .map(ext => `${o.inDic.basename}${ext}`);
  o.inDic.foundfiles = fs.readdirSync(o.inDic.inDir)
...
function readSDIfo() {
  ...
  fs.readFileSync(o.inDic.foundfiles.find(f => f.endsWith('.ifo')), o.inCoding)
    .replace(/^\uFEFF/, '')
    .split(/[\n\r]+/)
    .forEach((line) => {
      const [, k, v] = line.match(o.inDic.dirRE)||[];
...
function readSDSyn() {
  ...
  o.inDic.synBf = fs.readFileSync(o.inDic.foundfiles.find(f => f.endsWith('.syn')));
  ...
  while (synOffset < o.inDic.synBf.length) {
    ...
    o.inDic.syns.push([
      Buffer.from(synBf).toString(o.inCoding),
      i,
    ]);
  }
...
function readSDDic() {
  ...
  if (o.inDic.foundfiles.some(f => f.endsWith('.idx'))) {
    o.inDic.idxBf = fs.readFileSync(o.inDic.foundfiles.find(f => f.endsWith('.idx')));
  } else {
  ...
  if (o.inDic.foundfiles.some(f => f.endsWith('.dict'))) {
    o.inDic.dictBf = fs.readFileSync(o.inDic.foundfiles.find(f => f.endsWith('.dict')));
  } else {
