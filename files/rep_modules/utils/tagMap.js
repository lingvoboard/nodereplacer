...
module.exports = {
  setMap: function (str, obj, opt) {
      ...
      if (node.hasOwnProperty('tagName')) {
        ...
        if (options.map && attributes.length) {
          ...
          for (let attr of attributes) {
            ...
            tabTags[node.tagName].Attributes = [
              ...new Set(tabTags[node.tagName].Attributes)
            ]
            ...
            if (attributes.length > 1 && nonClassAttr.length) {
              if (!tabAttrs[attr.name].Attributes) {
                tabAttrs[attr.name].Attributes = (classVal
                  ? nonClassAttr.concat('class')
                  : nonClassAttr
                )
            ...
            if (classVal) {
              ...
              tabAttrs[attr.name].Classes = [
                ...new Set(tabAttrs[attr.name].Classes)
              ]
            }
          }
          ...
          if (classVal) {
            ...
            tabTags[node.tagName].Classes = [
              ...new Set(tabTags[node.tagName].Classes)
            ]
            ...
            for (let i = 0; i < localClassList.length; i++) {
              ...
              tabClassList[localClassList[i]].Tags = [
                ...new Set(tabClassList[localClassList[i]].Tags)
              ]
              if (nonClassAttr.length) {
                ...
                tabClassList[localClassList[i]].Attributes = [
                  ...new Set(
                    tabClassList[localClassList[i]].Attributes.concat(
                      nonClassAttr
                    )
                  )
                ]
    ...
    if (options.selectors || options.entries) {
      ...
      this.selectorMapArr = [
        ...new Set(this.selectorMapArr.concat(selectorArr))
      ]
      ...
      if (options.entries) {
        ...
        this.entriesMapObj[selectorArr.length].push([
          this.count,
          selectorHashArr
        ])
      }
  ...
  getMap: function (type) {
    ...
    if (options.map) {
      ...
      if (
        typeof type === 'string' &&
        ~keysArr.indexOf(type.charAt(0).toUpperCase() + type.slice(1))
      ) {
        result += stringifyObject(
          tagMapObj[type.charAt(0).toUpperCase() + type.slice(1)],
          { indent: '    ' }
        )
      } else {
    ...
    if (options.entries) {
      ...
      objKeys.forEach((key, i) => {
        artLinesArr[i] =
          options.limit && options.random
            ? (shuffleArray(entriesMapObj[key]), entriesMapObj[key])
            : entriesMapObj[key]
        ...
        entriesMap[
          ...
          ? lineNrArr.slice(0, options.limit).sort((a, b) => a - b)
          : lineNrArr
      })
      ...
      let duplicates = selectorHashArr.reduce(function (acc, el, i, arr) {
        ...
        if (
          arr.indexOf(el) !== dup &&
          selectorMapArr[i] !== selectorMapArr[dup]
        ) {
      ...
      throughArticles: for (let i = 0; i < artLinesArr.length; i++) {
        ...
        for (let j = 0; j < artLineArr.length; j++) {
          if (selectorHashArr.length) {
            ...
            const intersect = selectorHashArr.filter(n =>
              artCell[1].includes(n)
            )
            if (intersect.length) {
              minSetArr.push(artCell[0])
              const difference = selectorHashArr.filter(
                n => !intersect.includes(n)
              )
      ...
      entriesMap['Minimal test set (ALL selectors)'] = minSetArr.sort(
        (a, b) => a - b
      )

      result +=
        stringifyObject(entriesMap, {
          indent: '    ',
