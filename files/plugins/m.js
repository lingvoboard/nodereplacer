...
for (let v of hw)
{
	...
	o.BigArr.push([o.BigArr.length, ts, o.dsl[1].replace(/\n+$/, ""), v[0], v[1], [o.dsl[3]], 0]);
}
...
function write(cmd)
{
	...
	for (let i = 0; i < o.BigArr.length; i++)
	{
		...
		if ((/^-(a1|a2|l1|l2|r1|r2)$/i.test(process.argv[3])) && (o.hash[o.BigArr[i][1]] !== undefined))
		{
		...
		for (let j = 0; j < o.BigArr[i][5].length; j++)
		{
			...
			if ((o.BigArr[i][5].length > 1) && (/^-(a|a1|a2)$/i.test(cmd)))
			{
				...
				fs.writeSync(output, "\t" + '{{Arabic}}' + n + '{{/Arabic}}' + "\n" + o.BigArr[i][5][j].join("\n") + "\n", null, o.out_encoding);
			...
			else if ((o.BigArr[i][5].length > 1) && (/^-(l|l1|l2)$/i.test(cmd)))
			{
				...
				if (j === 0)
				{
					fs.writeSync(output, o.BigArr[i][5][j].join("\n") + "\n", null, o.out_encoding);		
				}
				else
				{
					fs.writeSync(output, "\t" + '————————' + "\n" +  o.BigArr[i][5][j].join("\n") + "\n", null, o.out_encoding);		
			...
			else if ((o.BigArr[i][5].length > 1) && (/^-(r|r1|r2)$/i.test(cmd)))
			{
				...
				fs.writeSync(output, "\t" + '{{Roman}}' + romanize(n) + '{{/Roman}}' + "\n" + o.BigArr[i][5][j].join("\n") + "\n", null, o.out_encoding);		
	...
	
	{
		...
		{
			fs.writeSync(output, o.DubArr[i][4] + "\n" + o.DubArr[i][2] + "\n", null, o.out_encoding);		
		}
