...
function onstart()
{
	...
	if (process.argv.length === 5)
	{
		...
		o.notEscapedDSLTagRE = new RegExp(
		  `(?<=${o.startOfString}|${o.notEscapeSymbol}|${o.escapedEscapeSymbols})${o.DSLTag}`, 'g'
		);
		...
		[...o.DSLtoHTMLmap.entries()].forEach(
		  (pair) => { o.DSLtoHTMLmap.set(`/${pair[0]}`, `/${pair[1]}`); }
		);
...
function dsl2html2dsl(str)
{
	...
	let doc = o.parse5.parse('<!DOCTYPE html><html><head></head><body></body></html>',  {
...
if ((process.argv.length === 6) && (process.argv[3] === '-gls'))
{
	...
	let doc = o.parse5.parse('<!DOCTYPE html><html><head></head><body>' + o.gls[1] + '</body></html>');
...
else
{
	...
	s = o.dsl[0].join("\n").trim() + "\n" + s.replace(/&#(\d+);/g, function(u, m1){let c = String.fromCodePoint(m1); return c});
