...
function onstart()
{
	...
	else if (process.argv.length === 6 && /^-t/.test(process.argv[3]) && ((/^-t\d+$/.test(process.argv[3])) || (checkparam(process.argv[3])[0])))
	{
	...
	else if (process.argv.length === 6 && /^-d/.test(process.argv[3]) && ((/^-d\d+$/.test(process.argv[3])) || (checkparam(process.argv[3])[0])))
	{
	...
	else if (process.argv.length === 6 && /^-g/.test(process.argv[3]) && ((/^-g\d+$/.test(process.argv[3])) || (checkparam(process.argv[3])[0])))
	{
		...
		if (o.arr && o.arr.length > 0)
		{
			...
			else
			{
				...
				if (o.arr.length > 0)
				{
					...
					if (/^-g/.test(process.argv[3]))
					{
						fs.writeFileSync(o.outputfile, s + "\n\n", {encoding: o.out_encoding, flag: "w"});
					}
					else
					{
						fs.writeFileSync(o.outputfile, s + "\n", {encoding: o.out_encoding, flag: "w"});
