...
function onstart()
{
	...
	else
	{
		...
		fs.writeFileSync(process.argv[4], e.message, {encoding: 'utf8', flag: "w"});
...
function processpage(b)
{
	...
	try
	{
		...
		for (let i = 0; i < document.body.childNodes.length; i++)
		{
			...
			if ((document.body.childNodes[i].nodeType === 3) &&
			(/^[\(\)\{\}\[\]\.;,\$\?\:%]+$/.test(document.body.childNodes[i].nodeValue.replace(/\s/g, ""))))
			{
...
function escapeHTMLtags(code)
{
	code = code.replace(/(<[^<>]*>)/g, function(all, m1){
		return m1.replace(/([&<>])/g, (all, m1)=>{if (m1 === '<'){return '&lt;';}else if (m1 === '>'){return '&gt;';}else{return '&amp;';}});
	});  
